...
//! Describes a format of a source coverage, which is used to convert the coverages to `PixelCoverageFormat`.
enum class PixelCoverageSource : uint8_t {
  //! Coverages are packed, stored consecutively across a vector or multiple vectors.
  kPackedConsecutive = 0,
  //! Coverages are unpacked, stored consecutively across a vector or multiple vectors
  kUnpackedConsecutive = 1,
  //! Coverages are unpacked, stored in each 64-bit low part of a vector.
  //!
  //! If the input vector is greater than 128 bits, then coverages occupy 64-bit lower bits of each 128-bit
  //! part of such vector. This kind of coverage source is often the result of using AVX2 or AVX512 packing
  //! with saturation.
  kUnpackedHalfLo128 = 2
};
