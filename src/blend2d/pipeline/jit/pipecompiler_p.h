...
// TODO: The intention is to have it as independent as possible, so remove this in the future!
...
enum class OpcodeMV : uint32_t {
  ...
  kMaxValue = kStoreCvtsN_U64ToU32
};
...
public:
  ...
  //! Tests whether the target SIMD ISA provides instructions with non-destructive destination (AVX+).
  BL_INLINE_NODEBUG bool hasNonDestructiveDst() const noexcept { return hasAVX(); }
  ...
  BL_INLINE_NODEBUG bool hasNonDestructiveDst() const noexcept { return true; }
  ...
  SimdWidth maxSimdWidthFromCpuFeatures() noexcept;
  void initSimdWidth(SimdWidth simdWidth) noexcept;
  ...
  BL_INLINE_NODEBUG SimdWidth simdWidth() const noexcept { return _simdWidth; }

  //! Returns whether the compiler and all parts use 256-bit SIMD.
  BL_INLINE_NODEBUG bool use256BitSimd() const noexcept { return _simdWidth >= SimdWidth::k256; }
  //! Returns whether the compiler and all parts use 512-bit SIMD.
  BL_INLINE_NODEBUG bool use512BitSimd() const noexcept { return _simdWidth >= SimdWidth::k512; }
  ...
  BL_INLINE_NODEBUG uint32_t simdMultiplier() const noexcept { return _simdMultiplier; }

  BL_INLINE_NODEBUG SimdWidth simdWidthOf(DataWidth dataWidth, uint32_t n) const noexcept { return SimdWidthUtils::simdWidthOf(simdWidth(), dataWidth, n); }
  BL_INLINE_NODEBUG uint32_t regCountOf(DataWidth dataWidth, uint32_t n) const noexcept { return SimdWidthUtils::regCountOf(simdWidth(), dataWidth, n); }

  BL_INLINE_NODEBUG SimdWidth simdWidthOf(DataWidth dataWidth, PixelCount pixelCount) const noexcept { return SimdWidthUtils::simdWidthOf(simdWidth(), dataWidth, pixelCount.value()); }
  BL_INLINE_NODEBUG uint32_t regCountOf(DataWidth dataWidth, PixelCount pixelCount) const noexcept { return SimdWidthUtils::regCountOf(simdWidth(), dataWidth, pixelCount.value()); }
...
#if defined(BL_JIT_ARCH_A64)
  inline Vec simdVecZero(const Vec& similarTo) noexcept { return simdVecConst(&ct.i_0000000000000000, similarTo); }
#endif // BL_JIT_ARCH_A64
  ...
  void emit_mv(OpcodeMV op, const Mem& dst_, const Vec& src_, uint32_t alignment) noexcept;
  void emit_mv(OpcodeMV op, const Mem& dst_, const OpArray& src_, uint32_t alignment) noexcept;
  ...
  DEFINE_OP_2V(v_cvt_i8_to_i16, OpcodeVV::kCvtI8ToI16)
  DEFINE_OP_2V(v_cvt_u8_to_u16, OpcodeVV::kCvtU8ToU16)
  DEFINE_OP_2V(v_cvt_i8_to_i32, OpcodeVV::kCvtI8ToI32)
  DEFINE_OP_2V(v_cvt_u8_to_u32, OpcodeVV::kCvtU8ToU32)
  DEFINE_OP_2V(v_cvt_i16_to_i32, OpcodeVV::kCvtI16ToI32)
  DEFINE_OP_2V(v_cvt_u16_to_u32, OpcodeVV::kCvtU16ToU32)
  DEFINE_OP_2V(v_cvt_i32_to_i64, OpcodeVV::kCvtI32ToI64)
  DEFINE_OP_2V(v_cvt_u32_to_u64, OpcodeVV::kCvtU32ToU64)
  ...
  void x_assign_unpacked_alpha_values(Pixel& p, PixelFlags flags, const Vec& vec) noexcept;
  ...
  BL_NOINLINE void xMovzxBW_LoHi(const Vec& d0, const Vec& d1, const Vec& s) noexcept {
    ...
    if (hasSSE4_1()) {
      if (d0.id() == s.id()) {
        ...
        v_cvt_u8_to_u16(d0, d0);
        v_cvt_u8_to_u16(d1, d1);
      }
      else {
        v_cvt_u8_to_u16(d0, s);
  ...
  inline void v_expand_alpha_16(const Dst& d, const Src& s, uint32_t useHiPart = 1) noexcept {
    ...
    if (useHiPart) {
      if (hasAVX() || (hasSSSE3() && d == s)) {
        v_swizzlev_u8(d, s, simdConst(&commonTable.pshufb_32xxxxxx10xxxxxx_to_3232323210101010, Bcst::kNA, d));
      }
      else {
  ...
  BL_NOINLINE void xExtractUnpackedAFromPackedARGB32_2(const Vec& d, const Vec& s) noexcept {
    ...
    v_swizzlev_u8(d, s, simdConst(&commonTable.pshufb_xxxxxxxx1xxx0xxx_to_z1z1z1z1z0z0z0z0, Bcst::kNA, d));
  }
  ...
  BL_NOINLINE void xExtractUnpackedAFromPackedARGB32_4(const Vec& d0, const Vec& d1, const Vec& s) noexcept {
    ...
    if (d0.id() == s.id()) {
      v_swizzlev_u8(d1, s, simdConst(&ct.pshufb_1xxx0xxxxxxxxxxx_to_z1z1z1z1z0z0z0z0, Bcst::kNA, d1));
      v_swizzlev_u8(d0, s, simdConst(&ct.pshufb_xxxxxxxx1xxx0xxx_to_z1z1z1z1z0z0z0z0, Bcst::kNA, d0));
    }
    else {
      v_swizzlev_u8(d0, s, simdConst(&ct.pshufb_xxxxxxxx1xxx0xxx_to_z1z1z1z1z0z0z0z0, Bcst::kNA, d0));
      v_swizzlev_u8(d1, s, simdConst(&ct.pshufb_1xxx0xxxxxxxxxxx_to_z1z1z1z1z0z0z0z0, Bcst::kNA, d1));
  ...
  BL_NOINLINE void xPackU32ToU16Lo(const Vec& d0, const Vec& s0) noexcept {
    ...
    if (hasSSE4_1()) {
      ...
      v_swizzlev_u8(d0, s0, simdConst(&commonTable.pshufb_xx76xx54xx32xx10_to_7654321076543210, Bcst::kNA, d0));
    }
    else {
