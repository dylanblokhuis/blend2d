...
void FetchSimplePatternPart::prefetchAccX() noexcept {
  ...
  if (isAlphaFetch()) {
    ...
    if (isPatternFx()) {
      ...
      pc->v_cvt_u8_to_u16(f->pixL, f->pixL);
      ...
      pc->v_cvt_u8_to_u16(pixL, pixL);
      pc->v_cvt_u8_to_u16(pixT, pixT);
...
void FetchSimplePatternPart::fetch(Pixel& p, PixelCount n, PixelFlags flags, PixelPredicate& predicate) noexcept {
  ...
  if (!blTestFlag(flags, PixelFlags::kPA_UA_UI | PixelFlags::kPC_UC)) {
    if (p.isRGBA32())
  ...
  if (isAlignedBlit()) {
    pc->x_fetch_pixel(p, n, flags, format(), mem_ptr(f->srcp1), Alignment(4), predicate);
    if (predicate.empty())
  ...
  switch (n.value()) {
    case 1: {
      ...
      if (isPatternAligned()) {
        pc->x_fetch_pixel(p, PixelCount(1), flags, format(), mem_ptr(f->srcp1, idx, _idxShift), Alignment(4));
        advanceXByOne();
      }
      ...
      if (isPatternFy()) {
        if (isAlphaFetch()) {
          ...
          pc->x_fetch_unpacked_a8_2x(pixA, format(), mem_ptr(f->srcp1, idx, _idxShift), mem_ptr(f->srcp0, idx, _idxShift));
          ...
          pc->v_cvt_u8_to_u16(pix0, pix0);
          pc->v_cvt_u8_to_u16(pix1, pix1);
        ...
        if (p.isRGBA32()) {
          if (blTestFlag(flags, PixelFlags::kPC)) {
            ...
            switch (format()) {
              case FormatExt::kPRGB32:
              case FormatExt::kXRGB32: {
                pc->v_cvt_u8_to_u16(uc0, mem);
                pc->v_cvt_u8_to_u16(uc1, mem.cloneAdjusted(8));
                break;
              }
              case FormatExt::kA8: {
                pc->v_loadu32(uc0, mem);
                pc->v_interleave_lo_u8(uc0, uc0, uc0);
                pc->v_cvt_u8_to_u16(uc0, uc0);
    ...
    case 1: {
      switch (fetchType()) {
        case FetchType::kPatternAffineNNAny: {
          ...
          pc->x_fetch_pixel(p, PixelCount(1), flags, format(), mem_ptr(texPtr, texOff, _idxShift), Alignment(4));
          clampVIdx32(vIdx, f->px_py, kClampStepA_NN);
        ...
        case FetchType::kPatternAffineNNOpt: {
          ...
          pc->add(texPtr, texPtr, f->srctop);
          pc->v_max_i16(vIdx, vIdx, f->minx_miny);
          pc->x_fetch_pixel(p, PixelCount(1), flags, format(), mem_ptr(texPtr), Alignment(4));
    ...
    case 4: {
      switch (fetchType()) {
        ...
        case FetchType::kPatternAffineNNOpt: {
          ...
          pc->v_mhadd_i16_to_i32(vIdx, vIdx, f->vAddrMul);
          FetchUtils::x_gather_pixels(pc, p, PixelCount(4), format(), flags, mem_ptr(f->srctop), vIdx, 0, IndexLayout::kUInt32, [&](uint32_t step) noexcept {
            switch (step) {
