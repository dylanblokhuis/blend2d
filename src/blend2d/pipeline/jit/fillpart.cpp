...
FillAnalyticPart::FillAnalyticPart(PipeCompiler* pc, FetchPixelPtrPart* dstPart, CompOpPart* compOpPart) noexcept
  ...
  _maxSimdWidthSupported = blMin(SimdWidth::k256, SimdWidth::kMaxPlatformWidth);
}
...
void FillAnalyticPart::compile(const PipeFunction& fn) noexcept {
  ...
  Vec vecZero = pc->newV128("vec_zero");                     // Reg/Tmp.

  Pixel dPix("d", pixelType);                                // Reg.

  VecArray m;                                                // Reg.
  VecArray compCov;                                          // Tmp (only for passing coverages to the compositor).
  FetchUtils::initVecCoverage(pc, m, PixelCount(maxPixels), pc->simdWidth(), pixelType, coverageFormat);
  ...
  if (vProcPixelCount == 8u) {
    ...
    FetchUtils::passVecCoverage(compCov, m, PixelCount(8), pixelType, coverageFormat);
    ...
    FetchUtils::expandCoverages(pc, m, PixelCount(8), pixelType, coverageFormat, PixelCoverageSource::kUnpackedHalfLo128);
    ...
    FetchUtils::passVecCoverage(compCov, m, PixelCount(4), pixelType, coverageFormat);
  ...
  if (vProcPixelCount == 4u) {
    ...
    calcMasksFromCells(m[0], cov, fillRuleMask, globalAlpha);
    normalizeCoverages(cov);
    FetchUtils::expandCoverages(pc, m, PixelCount(4), pixelType, coverageFormat, PixelCoverageSource::kUnpackedConsecutive);
  ...
  // ---------------------------------
    ...
    if (pixelType == PixelType::kRGBA32) {
      ...
      shiftOrRotate(m[0], m[0], 2);                          //   m0[15:0] = [??, m[15:2]]
    }
    else if (pixelType == PixelType::kA8) {
      pc->bind(L_VLoop_Step);                                // L_VLoop_Step:
  ...
  if (maxPixels >= 4u) {
    ...
    if (pixelType == PixelType::kRGBA32) {
      ...
      pc->store_u8(mem_ptr(dstPtr), dPix.sa);
      pc->add(dstPtr, dstPtr, dstBpp);                       //   dstPtr += dstBpp;
      pc->v_srlb_u128(m128, m128, 2);                        //   m0[7:0] = [0, m[7:1]]
      compOpPart()->nextPartialPixel();                      //   <CompOpPart::nextPartialPixel>
      dPix.resetAllExceptTypeAndName();
      pc->j(L_VTail_Cont, sub_nz(i, 1));                     //   if (--i) goto L_VTail_Cont;
...
void FillAnalyticPart::calcMasksFromCells(const Vec& dst_, const Vec& cov, const Vec& fillRuleMask, const Vec& globalAlpha) noexcept {
  ...
  pc->v_srai_i32(dst, cov, A8Info::kShift);
  ...
  if (pc->hasSSE4_1()) {
    ...
    // us a register.
    pc->v_min_u32(dst, dst, pc->simdConst(&ct.i_0000020000000200, Bcst::kNA, dst));
  ...
  pc->v_srai_i32(dst, cov, A8Info::kShift + 1);
  pc->v_and_i32(dst, dst, fillRuleMask);

  pc->v_sub_i32(dst, dst, pc->simdConst(&ct.i_0000010000000100, Bcst::k32, dst));
  pc->v_abs_i32(dst, dst);
  pc->v_min_u32(dst, dst, pc->simdConst(&ct.i_0000010000000100, Bcst::kNA, dst));

  pc->v_mul_u16(dst, dst, globalAlpha);
  pc->cc->shrn(dst.h4(), dst.s4(), 8);
#endif
}

void FillAnalyticPart::shiftOrRotate(const Vec& dst, const Vec& src, uint32_t n) noexcept {
#if defined(BL_JIT_ARCH_X86)
